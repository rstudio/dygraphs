...
You can specify an alternate color pallette for series lines using the `colors` option. For example, to choose a pallette from [RColorBrewer](http://cran.rstudio.com/web/packages/RColorBrewer/index.html):
...
All of the options above can also be set on a per-series basis usign the `dySeries` function. For example:
...
```{r}
dygraph(ldeaths, main = "Deaths from Lung Disease (UK)") %>%
  dySeries("V1", strokeWidth = 2, strokePattern = c(5, 2))
```
