...
The `independentTicks` option can be used to determine which axis is primary (and therefore which axis grid lines are aligned with). In order to disply the secondary axis scale at least one of the two axes must specify independentTicks = TRUE. Possible combinations include: 
...
In this example we specify that the y2 axis has indepenent ticks (resulting in a more natural value scale for the axis labels). We also add a label to each Y axis.
